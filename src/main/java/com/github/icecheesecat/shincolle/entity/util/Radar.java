...
public class Radar extends ShipTickableAction {
    ...
    private void normalScan(Level level) {
        if (!level.isClientSide) {
            List<Entity> list = level.getEntities(this.ship, this.ship.getBoundingBox().inflate(this.ship.getShipAttrs().getLos()), isEnemy());

            this.targets.clear();
            double r = Math.pow(this.ship.getShipAttrs().getLos(), 2);
            for (var e: list) {
    ...
    private static Predicate<Entity> isEnemy() {
        return (e) -> {
            if (e.getClass().isInstance(Monster.class)) {
                return true;
            }
            else {
                return false;
