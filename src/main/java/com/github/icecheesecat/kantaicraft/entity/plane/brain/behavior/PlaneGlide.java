...
public class PlaneGlide extends PhysicalMovementBehavior {
    ...
    public PlaneGlide(double speed, double turnAcceleration) {
        super(ImmutableMap.of(ModBrain.OWNERSHIP.get(), MemoryStatus.VALUE_PRESENT), speed, turnAcceleration);
    }
    ...
    protected void start(ServerLevel serverLevel, BasicEntityPlane basicEntityPlane, long gametime) {
        Entity owner = serverLevel.getEntity(basicEntityPlane.getBrain().getMemory(ModBrain.OWNERSHIP.get()).get());
