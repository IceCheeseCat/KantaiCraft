...
public class DefaultStats {
    ...
    public DefaultStats addStats(Map<EquipmentStatType, Double> n_stats) {
        this.stats.putAll(n_stats);
        return this;
    }

    public DefaultStats addStat(EquipmentStatType statType, double val) {
    ...
    private static DefaultStats createCannon(int size, double missile_velocity, double range, long cooldown) {
        return new DefaultStats().addStats(Map.of(
    ...
    public static DefaultStats createSmallCannon(double missile_velocity, double range, long cooldown) {
        return createCannon(0, missile_velocity, range, cooldown);
    }

    public static DefaultStats createMediumCannon(double missile_velocity, double range, long cooldown) {
        return createCannon(1, missile_velocity, range, cooldown);
    }

    public static DefaultStats createLargeCannon(double missile_velocity, double range, long cooldown) {
        return createCannon(2, missile_velocity, range, cooldown);
    }

    public static final Map<Integer, DefaultStats> ALL_DEFAULT_VALUES = new HashMap<>();

    public static void register(int id, DefaultStats defaultStats) {
        if (ALL_DEFAULT_VALUES.containsKey(id)) {
            throw new IllegalStateException("Default value of " + id + " has already registered!");
        }
        ALL_DEFAULT_VALUES.put(id, defaultStats);
    }

    public static DefaultStats getById(int id) {
        if (!ALL_DEFAULT_VALUES.containsKey(id)) {
            throw new IllegalStateException("No default value for "+ id +" registered!");
        }

        return DefaultStats.ALL_DEFAULT_VALUES.get(id);
    }
